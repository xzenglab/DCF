function ScoreMatrix = DCF_mul_views(split_index,k, featureRank, networkRank, lambda, alpha, fname, indicators, frac)
% INPUT PARAMS:
...
%       network_features.features = network_features.features(:,indices);    
     ...
     % repeat to add weight in decision
%      string = load('dbn_gene_features.mat');
     ori = load('9origin_features.mat');
     features = [features ori.features];
     cross_view = load('DCCAE_features-[512,256,256,]-[512,256,256,]-random-tanh.mat');
     features = [features cross_view.X1_proj];
...
save 'DCF_multi_view.mat' 'ScoreMatrix'
